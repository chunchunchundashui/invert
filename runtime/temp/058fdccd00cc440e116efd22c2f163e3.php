<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"D:\phpStudy\WWW\invest\public/../application/index\view\yes\thanks.html";i:1589458676;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <title>Document</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        } 
        body, html {
            width: 100%;
            height: 100%;
        }

        .wrapper {
            padding: 0 15px;
            max-width: 750px;
            background-image: url(http://localhost/invest/public/static/index/img/bg.jpg);
            background-position: center center;
            background-size: cover;
            margin: 0 auto;
            height: 100%;
            box-shadow: 0px 0px 30px #888888;
        }

        p {
            margin: 0;
            padding: 0;
            text-align: center;
            padding-top: 46%;
            transform: translateY(-30%);
            font-size: 32px;
            color: #52b1e2;
        }
      div #copyright {
        margin-top: 36%;
        font-size: 14px;
      }
    </style>
  </head>
  <body>
    <div class="wrapper">
        <p>谢谢参与此次调查</p>
      <p id="copyright">四川新华电脑学院Document &copy;2020 版权所有<br />
        教师:陈雪冬<br />学生:马冲 岳长春 刘友刚</p>
    </div>
    <script src="http://localhost/invest/public/static/admin/style/jquery-3.4.1.min.js"></script>
    <script src="http://localhost/invest/public/static/admin/style/bootstrap.js"></script>
    <script src="http://localhost/invest/public/static/admin/style/jquery.js"></script>
    <!--Beyond Scripts-->
    <script src="http://localhost/invest/public/static/admin/style/beyond.js"></script>
    <script src="http://localhost/invest/public/static/lib/layer/layer.js"></script>
   		<script> 
javascript:window.history.forward(1); 
</script>
  </body>
</html>
